...
@test "Database sync: development, no existing site" {
  ...
  assert_equal "--alias-path=/app/drush/sites @govcms.prod sql:dump --gzip --extra-dump=--no-tablespaces --structure-tables-list=search_index,search_total,redirect_404,cache,cache_* --result-file=/tmp/sync.sql --skip-tables-key=common -y" "$(mock_get_call_args "${mock_drush}" 2)"
...
@test "Database sync: development, alias overrides" {
  ...
  assert_output_contains "[info]: Loading database using drush"
  assert_output_contains "[info]: Preparing database sync"
  assert_equal "--alias-path=/app/drush/othersites @govcms.override sql:dump --gzip --extra-dump=--no-tablespaces --structure-tables-list=search_index,search_total,redirect_404,cache,cache_* --result-file=/tmp/sync.sql --skip-tables-key=common -y" "$(mock_get_call_args "${mock_drush}" 2)"
